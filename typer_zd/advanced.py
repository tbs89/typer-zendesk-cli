...
def apply_macro_to_ticket(zenpy_client, ticket_id: int, macro_id: int):
    """Applies a macro to a ticket and returns the result."""
...
def run_macro_on_tickets(file_name: str, environment: str):
    """Applies macros to tickets based on a CSV file using pandas."""
...
def apply_tags_to_tickets(file_name: str, environment: str):
    """Applies tags to tickets based on a CSV file."""
    ...
    if not any(col for col in df.columns if col.startswith('tag_')):
        ...
        print(f"[bold blue]Download the CSV template and try again: https://github.com/tbs89/typer_zd/blob/main/docs/templates/apply_tags_template.csv[/bold blue]")
        return
